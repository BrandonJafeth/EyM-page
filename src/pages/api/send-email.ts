...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!apiKey) {
      ...
      return new Response(
        JSON.stringify({ message: "Error interno: Falta API Key" }),
        { status: 500, headers: { "Content-Type": "application/json" } }
      );
    }

    const resend = new Resend(apiKey);
    const body = await request.json(); // May throw if empty
                    ...
                    <tr>
                      <td style="background-color: #091723; padding: 15px; text-align: center;">
                        <p style="color: #9ca3af; font-size: 10px; margin: 0;">
                          ...
                          &copy; ${new Date().getFullYear()} EYM & Asociados.
                        </p>
                      </td>
                    </tr>
    ...
    if (emailToAdmin.error) {
      console.error("Error Admin Email:", emailToAdmin.error);
       // No retornamos error todavía, intentamos enviar el del cliente si es posible, o fallamos aquí.
       // Generalmente si falla el admin, es critico.
                  ...
                  <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); overflow: hidden;">
                    ...
                    <tr>
                      <td style="background-color: #091723; padding: 20px; text-align: center;">
                        <p style="color: #9ca3af; font-size: 12px; margin: 0;">&copy; ${new Date().getFullYear()} EYM & Asociados.</p>
                      </td>
                    </tr>
                  </table>
                </td>
    ...
    // Log intent but don't fail user request if auto-reply fails (e.g. invalid user email)
    ...
    // Return proper JSON even for unexpected crashes
    return new Response(
      JSON.stringify({
